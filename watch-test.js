...
const canvasEl=$('[data-watch-canvas]');
const statusEl=$('[data-watch-status]');
const startButton=$('[data-watch-start]');
const stopButton=$('[data-watch-stop]');
const streamIdEl=$('[data-watch-stream-id]');
const detailEl=$('[data-watch-detail]');

let client=null;
let starting=false;

class WatchMediaError extends Error{
  constructor(code,message){super(message);this.name='WatchMediaError';this.code=code;}
}

function objectValue(value){return value&&typeof value==='object'&&!Array.isArray(value)?value:null;}
function currentPath(){const file=location.pathname.split('/').filter(Boolean).at(-1)||'watch-test.html';return './'+file;}
function clearFlowToken(){sessionStorage.removeItem(FLOW_KEY);}
function setStatus(text,kind=''){if(statusEl){statusEl.textContent=text;statusEl.dataset.kind=kind;}}
...
async function consumeWatchGrant(){
  const token=sessionStorage.getItem(FLOW_KEY)||'';
  if(!TOKEN_RE.test(token))throw new Error('FLOW_MISSING');
  const response=await fetch(FLOW_URL,{
    method:'POST',
  ...
  const data=await response.json().catch(()=>({}));
  if(!response.ok||data.ok!==true)throw new Error(typeof data.code==='string'?data.code:'WATCH_FLOW_FAILED');
  const grant=data.watchGrant;
  if(!grant||typeof grant!=='object')throw new Error('WATCH_GRANT_INVALID');
  clearFlowToken();
  return grant;
}
...
function validMediaUrl(raw){
  ...
  if(url.protocol!=='wss:'||url.hostname!==NORTHFLANK_HOST||(url.port&&url.port!=='443')||url.username||url.password||url.pathname!=='/realtime/media'||url.search||url.hash){
...
function validateGrant(grant){
  if(!STREAM_ID_RE.test(String(grant.streamId||''))||String(grant.capability||'').length>12000||!CAPABILITY_RE.test(String(grant.capability||''))||!Number.isSafeInteger(Number(grant.expiresAt))||Number(grant.expiresAt)<=Date.now()||Number(grant.expiresAt)>Date.now()+11*60_000){
    throw new WatchMediaError('MEDIA_GRANT_INVALID','視聴接続情報を確認できません。');
  }
  return Object.freeze({
    streamId:String(grant.streamId),
    capability:String(grant.capability),
    expiresAt:Number(grant.expiresAt),
    mediaWebSocketUrl:validMediaUrl(String(grant.mediaWebSocketUrl||'')),
  });
}

function reconnectDelay(attempt){return Math.min(MAX_RECONNECT_DELAY_MS,250*(2**Math.min(attempt,3)));}
function safeBigIntToNumber(value){if(value>BigInt(Number.MAX_SAFE_INTEGER)||value<BigInt(Number.MIN_SAFE_INTEGER))return null;return Number(value);}
function mediaMicros(value,num,den){if(!Number.isSafeInteger(num)||num<=0||!Number.isSafeInteger(den)||den<=0)return null;return safeBigIntToNumber(value*BigInt(num)*1_000_000n/BigInt(den));}

function parseWire(raw){
  if(!(raw instanceof ArrayBuffer)||raw.byteLength<WIRE_HEADER_BYTES||raw.byteLength>MAX_WIRE_BYTES)return null;
  ...
  const timebaseNum=view.getUint32(20,false),timebaseDen=view.getUint32(24,false);
...
function parseAudioPCM(payload){
  if(payload.byteLength<AUDIO_HEADER_BYTES)return null;
  const view=new DataView(payload.buffer,payload.byteOffset,payload.byteLength);
  if(view.getUint32(0,false)!==AUDIO_MAGIC||view.getUint8(4)!==AUDIO_VERSION||view.getUint8(6)!==0||view.getUint8(7)!==0)return null;
  const channels=view.getUint8(5),sampleRate=view.getUint32(8,false),frames=view.getUint32(12,false);
  if(channels<1||channels>8||sampleRate<8000||sampleRate>384000||frames<1||frames>4096)return null;
...
function annexBNALUnits(data){
  ...
  while(i+3<=data.length){
    ...
    for(let j=header+1;j+3<=data.length;j++){
      if((j+4<=data.length&&data[j]===0&&data[j+1]===0&&data[j+2]===0&&data[j+3]===1)||(data[j]===0&&data[j+1]===0&&data[j+2]===1)){end=j;break;}
    }
    units.push(data.subarray(header,end));i=end;
  }
  return units;
}
function avcCodecFromAnnexB(data){
  for(const nal of annexBNALUnits(data)){
    if(nal.length>=4&&(nal[0]&0x1f)===7)return `avc1.${nal[1].toString(16).padStart(2,'0')}${nal[2].toString(16).padStart(2,'0')}${nal[3].toString(16).padStart(2,'0')}`;
  }
  return null;
}
function parseReady(value){
  if(value.type!=='media_ready'||value.protocol!==1||value.videoPayload!=='h264-annexb'||value.audioPayload!=='oap1-f32-planar'||typeof value.resync!=='boolean'||!Number.isSafeInteger(value.startCursor)||Number(value.startCursor)<0||!Number.isSafeInteger(value.latestCursor)||Number(value.latestCursor)<0)return null;
  return Object.freeze({resync:value.resync,startCursor:BigInt(Number(value.startCursor)),latestCursor:BigInt(Number(value.latestCursor))});
}

class WatchMediaClient{
  constructor(grant,canvas,status=null){
    ...
    this.scheduledAudio=new Set();
    this.endedHandler=null;
  }
  onEnded(handler){this.endedHandler=handler;}
  start(){if(!this.stopping)this.connect();}
  async enableAudio(){
    if(this.stopping||this.audioEnabled)return;
    const Ctx=window.AudioContext||window.webkitAudioContext;
    if(!Ctx)throw new WatchMediaError('AUDIO_UNAVAILABLE','このブラウザでは音声を再生できません。');
    const context=this.audioContext??new Ctx({latencyHint:'interactive'});
  ...
  async stop(){
    ...
    if(this.audioContext){await this.audioContext.close().catch(()=>undefined);this.audioContext=null;}
  ...
  connect(){
    ...
    this.authenticated=false;this.setStatus('配信へ接続しています。');
    ...
    socket.addEventListener('close',(event)=>{
      ...
      this.setStatus('配信へ再接続しています。');this.scheduleReconnect();
    });
    socket.addEventListener('error',()=>{if(socket===this.socket)this.setStatus('配信通信を再確認しています。');});
  }
  handleControl(socket,raw){
    if(raw.length>64000){socket.close(1008,'control message too large');return;}
    let payload=null;try{payload=objectValue(JSON.parse(raw));}catch{}
  ...
  commitCursor(cursor){this.lastCursor=cursor;this.expectedCursor=cursor+1n;this.allowKeyframeJump=false;}
  handleVideo(socket,packet){
    const timestampUs=mediaMicros(packet.pts,packet.timebaseNum,packet.timebaseDen);
    if(timestampUs===null||timestampUs<0||typeof VideoDecoder==='undefined'||typeof EncodedVideoChunk==='undefined'){socket.close(1003,'video decoder unavailable');return false;}
    if(packet.keyframe){
      ...
      if(!this.decoder||codec!==this.decoderCodec||this.decoder.state==='closed'){
        ...
        try{
          this.decoder=new VideoDecoder({output:(frame)=>this.queueVideoFrame(frame),error:()=>this.forceKeyframeReconnect('映像を再同期しています。')});
          this.decoder.configure({codec,optimizeForLatency:true,hardwareAcceleration:'no-preference'});
          this.decoderCodec=codec;
        }catch{
          this.resetVideoDecoder();socket.close(1003,'h264 decoder configuration failed');return false;
        }
      }
    }
    ...
    try{
      ...
      return true;
    }catch{this.forceKeyframeReconnect('映像を再同期しています。');return false;}
  }
  queueVideoFrame(frame){
    if(this.stopping){frame.close();return;}
    this.pendingFrames.push(frame);
    if(this.pendingFrames.length>90){this.pendingFrames.shift()?.close();this.forceKeyframeReconnect('映像遅延を再同期しています。');return;}
    this.pumpVideoFrames();
  }
  pumpVideoFrames(){
    if(this.renderTimer!==null||this.pendingFrames.length===0)return;
    const frame=this.pendingFrames[0],timestampUs=frame.timestamp;
    if(!Number.isFinite(timestampUs)||timestampUs<0){this.pendingFrames.shift()?.close();this.forceKeyframeReconnect('映像時刻を再同期しています。');return;}
    if(this.videoAnchorMediaUs===null){this.videoAnchorMediaUs=timestampUs;this.videoAnchorPerfMs=performance.now();}
    const targetMs=this.videoAnchorPerfMs+(timestampUs-this.videoAnchorMediaUs)/1000;
    const delay=targetMs-performance.now();
    if(delay>4){
      this.renderTimer=window.setTimeout(()=>{this.renderTimer=null;this.pumpVideoFrames();},Math.min(delay,50));return;
    }
  ...
  handleAudio(socket,packet){
    const pcm=parseAudioPCM(packet.payload),timestampUs=mediaMicros(packet.pts,packet.timebaseNum,packet.timebaseDen);
    ...
    const source=context.createBufferSource();source.buffer=buffer;source.connect(context.destination);this.scheduledAudio.add(source);
    source.addEventListener('ended',()=>{this.scheduledAudio.delete(source);try{source.disconnect();}catch{}},{once:true});
    try{source.start(context.currentTime+deltaSec,offsetSec);}catch{this.scheduledAudio.delete(source);try{source.disconnect();}catch{}}
    return true;
  }
  forceKeyframeReconnect(message){
    if(this.stopping)return;this.setStatus(message);this.resetMediaState(false);
    const socket=this.socket;if(socket&&socket.readyState<WebSocket.CLOSING)socket.close(1013,'media resync required');
  }
  scheduleReconnect(){
  ...
  clearScheduledAudio(){
    for(const source of this.scheduledAudio){try{source.stop();}catch{}try{source.disconnect();}catch{}}
    this.scheduledAudio.clear();
  }
  resetMediaState(keepCursor){
    this.resetVideoDecoder();this.clearScheduledAudio();this.videoAnchorMediaUs=null;this.videoAnchorPerfMs=0;this.expectedCursor=null;this.allowKeyframeJump=false;if(!keepCursor)this.lastCursor=0n;
  }
  finishStream(){
    if(this.stopping)return;this.stopping=true;
    if(this.reconnectTimer!==null){clearTimeout(this.reconnectTimer);this.reconnectTimer=null;}
    const socket=this.socket;this.socket=null;if(socket&&socket.readyState<WebSocket.CLOSING)socket.close(1000,'stream ended');
    this.resetMediaState(true);this.setStatus('配信が終了しました。');this.endedHandler?.();
  }
}
...
async function startWatching(){
  ...
  setStatus('視聴認可を確認しています。');
  try{
    const grant=await consumeWatchGrant();
    if(streamIdEl)streamIdEl.textContent=grant.streamId;
    client=new WatchMediaClient(grant,canvasEl,statusEl);
    client.onEnded(()=>setTimeout(()=>location.replace('./index.html'),700));
    await client.enableAudio();
    client.start();
    stopButton.disabled=false;
    setDetail('Northflank の orikuro-media-v1 から H.264 Annex-B 映像と OAP1 PCM 音声を受信しています。');
  }catch(error){
    client=null;
    startButton.disabled=false;
    setStatus('視聴開始に失敗しました。','error');
    setDetail(error instanceof Error?error.message:'WATCH_START_FAILED');
  }finally{
    starting=false;
  }
}
...
async function initialize(){
  setStatus('視聴利用状態を確認しています。');
  try{
    await checkFlow();
    if(gateEl)gateEl.textContent='';
    if(contentEl)contentEl.hidden=false;
    startButton.disabled=false;
    setStatus('視聴開始の準備ができました。');
    setDetail('視聴開始を押すと、現在配信中のストリームに対する視聴権限を確認して接続します。');
  }catch(error){
    clearFlowToken();
    if(gateEl)gateEl.textContent=error?.message==='FLOW_MISSING'?'このサービスを直接開くことはできません。':'視聴準備を完了できませんでした。';
    setTimeout(()=>location.replace('./index.html'),1400);
  }
}

startButton?.addEventListener('click',()=>{void startWatching();});
stopButton?.addEventListener('click',()=>{void stopWatching();});
window.addEventListener('pagehide',()=>{if(client)void client.stop();},{once:true});
