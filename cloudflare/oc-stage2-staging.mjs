...
const SUPABASE_ROUTER_URL = "https://mpuhgfbdkxmhynytwhzu.supabase.co/functions/v1/mail-system/submission";
...
async function deriveMatchToken(matchDigest, rootSecret) {
  ...
  try {
    ...
    const rootKey = await crypto.subtle.importKey("raw", rootBytes, { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
    derivedBytes = new Uint8Array(await crypto.subtle.sign("HMAC", rootKey, contextBytes));
    const matchKey = await crypto.subtle.importKey("raw", derivedBytes, { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
    const token = new Uint8Array(await crypto.subtle.sign("HMAC", matchKey, digestBytes));
    try { return toBase64Url(token); } finally { token.fill(0); }
  } finally {
    digestBytes.fill(0);
    rootBytes.fill(0);
    contextBytes.fill(0);
...
async function importStage2PublicKey() {
  const key = await crypto.subtle.importKey("jwk", STAGE2_PUBLIC_KEY_JWK, { name: "RSA-OAEP", hash: "SHA-256" }, false, ["encrypt"]);
...
async function encryptLayer(data, publicKey, kid, layer) {
  ...
  try {
    ...
    const ciphertext = await crypto.subtle.encrypt({ name: "AES-GCM", iv, additionalData: aad, tagLength: 128 }, aesKey, data);
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      [protectedEmail, matchToken] = await Promise.all([
        ...
        body.email.version === 4 ? deriveMatchToken(body.email.matchDigest, env.OC_SUBMISSION_ROUTER_SECRET) : Promise.resolve(null),
      ]);
    } catch {
